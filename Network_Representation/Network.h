...
public:
	...
	int size() { return _nodes_.size(); }
	void clear() { _nodes_.clear(); }
	bool empty() { return _nodes_.empty(); }

	//Observer Sub
	void subscribe_to_network(Interface_Network_Observer* observer)
	{
		if (observer != nullptr)
			_network_observers_.push_back(observer);
	}

	void unsubscribe_from_network(const Interface_Network_Observer* observer)
	{
		std::remove_if(_network_observers_.begin(), _network_observers_.end(), [observer](const Interface_Network_Observer* a) {return a == observer; });
	}
protected:
	std::unordered_map<int, T> _nodes_;
	std::vector<Interface_Network_Observer*> _network_observers_;
};