...
Secondary functionality:
	1.) Standertizing communications
		The framework should rely on a protocol that will make it easy to setup and use the framework.
	2.) Network security
...
1.) Network initilization:
	...
	The network should exist outside of the GUI and should be decoupled from the GUI. The network representation is used for all other functionalities.
		...
		-A Design to communicate with the GUI. a derived class from agent must implement Object class interface.
2.) Appending new agents to network
...
3.) Historical data collection, graphing, and analysis
	An SQL connection should be implemented a data base should be constructed.
		Required tables:
		-agent_table
...
4.) Display and means to debug distributed networks
	...
	SFML allows adding text easily. this feature should be used to display relevant data about agents. First feature that should be implemented is displaying text when hovering mouse of agent.
		Required:
		-Initilaly hovering mouse over agent will display its id.
5.) Standertizing communications
	A protocol should be developed and enforced. Messageing must rely on the protocol to allow easy reading. JSON or other options should be considered as viable options to communicate.
	The most fundumental messages should be in mind:
	-echo
	-request to join network
	-request to leave network
	-sent data message to neighbour
	-request environment data
	-self assignment data to server